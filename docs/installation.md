...
```toml
[dependencies]
animato = { version = "1.7.1", features = ["path", "physics", "color", "serde"] }
```
...
| Use case | Dependency |
|----------|------------|
| TUI or CLI app | `animato = "1.7.1"` |
| Bevy game | `animato = { version = "1.7.1", features = ["bevy"] }` |
| Browser rAF loop | `animato = { version = "1.7.1", features = ["wasm"] }` |
| Browser DOM helpers | `animato = { version = "1.7.1", features = ["wasm-dom"] }` |
| Leptos CSR app | `animato = { version = "1.7.1", features = ["leptos-csr"] }` |
| Dioxus web app | `animato = { version = "1.7.1", features = ["dioxus-web"] }` |
| Dioxus desktop app | `animato = { version = "1.7.1", features = ["dioxus-desktop"] }` |
| Yew CSR app | `animato = { version = "1.7.1", features = ["yew-csr"] }` |
| Yew agent coordination | `animato = { version = "1.7.1", features = ["yew-csr", "yew-agent"] }` |
| JavaScript or TypeScript app | `npm install @aarambhdevhub/animato-core` |
| Runtime DevTools | `animato = { version = "1.7.1", features = ["devtools"] }` |
| SVG paths and morphing | `animato = { version = "1.7.1", features = ["path"] }` |
| Drag and gestures | `animato = { version = "1.7.1", features = ["physics"] }` |
| GPU batches | `animato = { version = "1.7.1", features = ["gpu"] }` |
